...
void ${module_variant_name}_test_forward_backward(const std::string& device) {
  ...
  for (const auto& param : module->named_parameters()) {
    grad_dict.insert(param.key(), param.value().grad());
  }
  write_ivalue_to_file(
...
def _test_torch_nn_module_variant(test_params):
    ...
    def test_forward_backward(test_params):
      ...
      python_output = module(test_params.test_instance._get_input())
      ...
      for name, param in module.named_parameters():
        python_grad_dict[name] = param.grad
      print("python_grad_dict: ", python_grad_dict)

      # yf225 TODO: maybe we can compile the cpp source code altogether first, and then do the result checking
